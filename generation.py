...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        all_long_mem = model_kwargs.get("all_long_mem", None)
        idx = 0
        mem_key_states = None
        mem_value_states = None
        long_query_states = None
        is_first_generate_flag = True
        is_generate = False
        cache = DynamicCache()
        while self._has_unfinished_sequences(
            ...
            current_chunk = cur_len // segment_len
            current_len = cur_len % segment_len
            # position_ids = torch.arange(cur_len, dtype=torch.long, device=input_ids.device).unsqueeze(0).expand(input_ids.shape[0], -1)
            if (idx-current_chunk == 0 or (idx-current_chunk == -1 and current_len == 0)) and is_first_generate_flag:
                is_first_generate = True
                is_first_generate_flag = False
                is_generate = True
            
            if idx-current_chunk < 0:
                chunk_input_ids = input_ids[: ,idx*segment_len:(idx+1)*segment_len]
                chunk_attention_mask = model_kwargs['attention_mask'][: ,idx*segment_len:(idx+1)*segment_len]
                return_kv = True
                add_rag = True
                idx +=1
            else:
                ...
                chunk_input_ids = input_ids[: ,idx*segment_len:current_len + idx*segment_len]
                chunk_attention_mask = model_kwargs['attention_mask'][: ,idx*segment_len:current_len + idx*segment_len]
            
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(chunk_input_ids, chunk_attention_mask, is_first_generate, is_generate, cache,**model_kwargs)
            model_inputs['is_generate'] = is_generate
            model_inputs['is_first_generate'] = is_first_generate
            model_inputs['add_rag'] = add_rag
            model_inputs['all_mem_key_states'] = mem_key_states
            model_inputs['all_mem_value_states'] = mem_value_states
            model_inputs['all_long_query_states'] = long_query_states
            model_inputs['all_long_mem'] = all_long_mem
            model_inputs['return_kv'] = return_kv
            ...
            # print("allocated (GB):", torch.cuda.memory_allocated() / gb)
            # print("reserved  (GB):", torch.cuda.memory_reserved()  / gb)
            if synced_gpus and this_peer_finished:
                continue  # don't waste resources running the code we don't need

            if return_kv:
                mem_key_states = outputs.get('mem_key_states', None)
                mem_value_states = outputs.get('mem_value_states', None)
                cache = DynamicCache()
            long_query_states = outputs.get('mem_query_states', None)
