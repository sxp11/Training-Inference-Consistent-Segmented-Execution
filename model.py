...
class LocalLlamaFlashAttention2(LlamaFlashAttention2):
    ...
    def forward(
        ...
        if mem_key_states is None:
            ...
            second_q, second_indices, second_cu_q_lens_r, second_max_s = unpad_input(query_states[:,self.other_heads].transpose(1,2), attention_mask)
            ...
            first_q, first_indices, first_cu_q_lens_r, first_max_s = unpad_input(query_states[:,self.selected_heads].transpose(1,2), attention_mask)
...
class LongLlamaFlashAttention2(LlamaFlashAttention2):
    ...
    def forward(
        ...
        if mem_key_states is None:
            ...
            q, indices, cu_q_lens_r, max_s = unpad_input(query_states.transpose(1,2), attention_mask)
...
class LlamaModelWithMemory(LlamaModel):
    ...
    def forward(
        ...
        # )
        causal_mask = torch.ones((input_ids.shape[0], input_ids.shape[1]), dtype=torch.bool, device=inputs_embeds.device)
